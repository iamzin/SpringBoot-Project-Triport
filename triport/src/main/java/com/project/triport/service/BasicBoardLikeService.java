...
public class BasicBoardLikeService {
    ...
    @Transactional
    public ResponseDto CreateAndDeleteBasicBoardLike(Long basicId, User user) {
...
//        User user = ((PrincipalDetails) authentication.getPrincipal()).getUser();
        ...
        boolean isExist = basicBoardLikeRepository.existsByBasicBoardAndUser(basicBoard, user);

        if(isExist) {
            basicBoardLikeRepository.deleteByBasicBoardAndUser(basicBoard,user);
            basicBoard.updateLikeNum(-1);
            return new ResponseDto(true, "Basic 게시글 좋아요 취소");
        } else {
            BasicBoardLike basicBoardLike = new BasicBoardLike(basicBoard, user);
